...
// src/repositories/in-memory/in-memory-organizations-repository.ts
...
var InMemoryOrganizationsRepository = class {
  ...
  async findById(id) {
    const organization = await this.items.find((organization2) => organization2.id === id);
    if (!organization) {
      return null;
    }
    return organization;
  ...
  async findManyNearby(state, city) {
...
// src/use-cases/cases/authenticate.ts
...
var AuthenticateUseCase = class {
  ...
  async execute({ email, password }) {
    ...
    const doesPasswordMatches = await (0, import_bcryptjs.compare)(password, organization.password_hash);
...
// src/use-cases/tests/authenticate.spec.ts
...
describe("Authenticate Use Case", () => {
  ...
  it("should be able to authenticate", async () => {
    ...
    globalExpect(organization.id).toEqual(globalExpect.any(String));
  });
